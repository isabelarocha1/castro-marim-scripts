...
EXCEL_FILE_SEDIMENT = 'Physicochemical Parameters table for sediments.xlsx'

# Output directories for plots
OUTPUT_DIR = 'Plots_Analysis_Final'
WATER_PLOTS_DIR = os.path.join(OUTPUT_DIR, 'Water_Plots')
SEDIMENT_PLOTS_DIR = os.path.join(OUTPUT_DIR, 'Sediment_Plots')
...
SALTWORK_PALETTE = {
    ...
    'WRED': (0.17254901960784313, 0.6274503921569, 0.17254901960784313) # Green
}

# Create output directories if they don't exist
os.makedirs(WATER_PLOTS_DIR, exist_ok=True)
os.makedirs(SEDIMENT_PLOTS_DIR, exist_ok=True)
...
def load_and_process_file(file_path, file_type):
    ...
    for sheet_name in xls.sheet_names:
        if (file_type == 'Water' and sheet_name.startswith('W ')) or \
           (file_type == 'Sediment' and sheet_name.startswith('S ')):
...
def generate_time_series_plots(df_filtered, plot_type):
    ...
    output_path = WATER_PLOTS_DIR if plot_type == 'Water' else SEDIMENT_PLOTS_DIR
    ...
    for param in PARAMS_TO_ANALYZE:
        ...
        df_plot = df_filtered[(df_filtered['Type'] == plot_type) & 
                              (df_filtered['Saltwork'].isin(['1', '2', 'WRED']))].copy()
...
def generate_time_series_by_stage_plots(df_filtered, plot_type):
    ...
    df_plot_base = df_filtered[(df_filtered['Type'] == plot_type) &
                               (df_filtered['Saltwork'].isin(['1', '2', 'WRED'])) &
...
if __name__ == "__main__":
    ...
    df_sediment = load_and_process_file(EXCEL_FILE_SEDIMENT, 'Sediment')

    dfs_coleta = []
    if not df_water.empty:
        dfs_coleta.append(df_water)
    if not df_sediment.empty:
        dfs_coleta.append(df_sediment)
    ...
    if not dfs_coleta:
        ...
        print("\n--- Filtered Data (Saltworks 1, 2, and WRED only) ---")
        ...
        generate_time_series_plots(filtered_df, 'Sediment')
        generate_comparison_bar_plots(filtered_df, 'Sediment')
